...
- Leiden算法介绍
    - Leiden 算法是一种用于社区检测的高效算法，是 Louvain 算法的改进版本。它解决了 Louvain 算法可能产生的不连通社区问题，并提供了更高质量的社区划分。该算法由Leiden University的研究者在2019年提出，论文地址： https://arxiv.org/pdf/1810.08473
    - 该算法主要通过三个步骤实现社区聚类：
        1. 局部移动：优化节点社区归属
        2. 细化：改进初始社区划分
        3. 聚合：基于细化结果构建新的网络
    - 其实现过程如下：
        1. 初始化：每个节点被分配到单独的社区中
        2. 局部移动阶段：节点根据模块度增益被移动到相邻社区中
        3. 细化阶段：Leiden算法的关键创新点，确保所有的社区都是连通的
        4. 聚合阶段：基于细化后的社区构建网络的缩减版本
        5. 迭代：重复上述步骤直到模块度不再显著提升
  - 其中，**模块度（Modularity）​**是衡量网络社区划分质量的核心指标，由Newman和Girvan在2004年提出。它通过比较**实际社区内边数**与**随机网络预期边数**的差异来评估社区结构的强度。
